...
BEGIN
  ...
  -- Generate base URL
  v_base_url := current_setting('app.base_url', true) || '/flow/' || v_campaign_id::text;
  IF v_base_url IS NULL OR v_base_url = '/flow/' || v_campaign_id::text THEN
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_flows_campaign_id ON flows(campaign_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_flows_created_by ON flows(created_by);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_campaigns_brand_id ON campaigns(brand_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_brands_user_id ON brands(user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_qr_codes_flow_id ON qr_codes(flow_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_flow_content_flow_id ON flow_content(flow_id);

-- Add validation constraints
ALTER TABLE flows ADD CONSTRAINT flows_name_not_empty CHECK (length(trim(name)) > 0);
ALTER TABLE campaigns ADD CONSTRAINT campaigns_name_not_empty CHECK (length(trim(name)) > 0);
ALTER TABLE brands ADD CONSTRAINT brands_name_not_empty CHECK (length(trim(name)) > 0);
