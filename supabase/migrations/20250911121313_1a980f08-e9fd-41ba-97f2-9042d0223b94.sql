-- Fix ambiguous column reference in admin_get_all_brands function
...
AS $function$
BEGIN
  -- Check if user is master_admin - qualify the user_id reference
  IF NOT EXISTS(SELECT 1 FROM public.profiles p WHERE p.user_id = auth.uid() AND p.role = 'master_admin') THEN
