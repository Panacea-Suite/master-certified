-- Add is_test column to campaigns, batches, and qr_codes tables
ALTER TABLE campaigns ADD COLUMN is_test boolean NOT NULL DEFAULT false;
ALTER TABLE batches ADD COLUMN is_test boolean NOT NULL DEFAULT false;
ALTER TABLE qr_codes ADD COLUMN is_test boolean NOT NULL DEFAULT false;

-- Create RPC function to create ephemeral campaign from template
...
BEGIN
  ...
  unique_code := 'test_' || encode(gen_random_bytes(8), 'hex');
...
-- Create cleanup function for ephemeral campaigns
CREATE OR REPLACE FUNCTION public.cleanup_ephemeral_campaigns(days_old integer DEFAULT 7)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  -- Delete old ephemeral campaigns and their child records
  ...
  SELECT count(*) INTO deleted_count FROM deleted_campaigns;
